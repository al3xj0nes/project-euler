...
namespace project_euler.Solutions
{
    public class Solution_08 : Solution
    {
        ...
        {
            Task task = Task.Run(() => ProductAndDigits(token, progress));
            await task;
        }

        private void ProductAndDigits(CancellationToken token, IProgress<int> progress = null)
        {
            ...
            for (int index = 0; index <= inputList.Length - adjacentCount; index++)
            {
                ...
                if (progress != null)
                {
                    double percent = 100 * (double)index / (inputList.Length - adjacentCount);
            ...
            Answer = $"Largest product = {largestProduct} from adjacent values {products}";
        }
    }
}
